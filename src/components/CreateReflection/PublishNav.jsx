      ...
      const publish = async () => {
        if(location.pathname.includes("/edit/")){
          ...
          try {
            ...
            let res = await fetch(`http://api.quotera.kennyparedes.com/reflections/${reflectionID}`, requestOptions);
          ...
          try {
            ...
            let res = await fetch(`http://api.quotera.kennyparedes.com/reflections/`, requestOptions);
            let resStatus = res.status;
            if(resStatus===401 || resStatus===403 || resStatus===404) {
              alert('Unauthorized')
              navigate('/');
